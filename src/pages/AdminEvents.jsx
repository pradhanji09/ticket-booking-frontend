...
export default function AdminEvents() {
  ...
  const resetForm = () => {
    setForm({ name: '', description: '', venue: '', eventDate: '', pricePerSeat: '' })
  }
  ...
  const handleEditClick = (event) => {
    ...
    setForm({
      ...
      pricePerSeat: event.pricePerSeat ? (event.pricePerSeat / 100).toString() : '',
    })
  }
  ...
  const handleCancelEvent = async (id) => {
    ...
    try {
      ...
      if (res.data.success) {
        setActionMessage(
          res.data.data.message || `Event cancelled. Refunded bookings: ${res.data.data.refundedBookings}`
        )
        fetchEvents(page)
      }
    } catch (err) {
      setActionMessage(err.response?.data?.error || 'Failed to cancel event')
    }
  }
  ...
  const handleBulkCreateSeats = async (e, eventId) => {
    ...
    setActionMessage('')
    try {
      const res = await API.post(`/api/events/${eventId}/seats/bulk`, {
        count: Number(seatForm.count),
        prefix: seatForm.prefix,
      })
      if (res.data.success) {
        setActionMessage(`Created ${res.data.data.created} seats`)
        setSeatFormEventId(null)
        fetchEvents(page)
      }
    } catch (err) {
      setActionMessage(err.response?.data?.error || 'Failed to create seats')
    }
  }
          ...
          {events.map((event) => (
            <tr key={event.id}>
              ...
              <td>
                ...
                <button
                  onClick={() =>
                    setSeatFormEventId(seatFormEventId === event.id ? null : event.id)
                  }
                  style={{ marginLeft: '5px' }}
                >
                  Manage Seats
                ...
                {seatFormEventId === event.id && (
                  <form
                    ...
                    style={{ marginTop: '5px', padding: '5px', border: '1px solid #ccc' }}
                  >
                    <div>
                      <label>Count: </label>
                      <input
                        ...
                        onChange={(e) => setSeatForm({ ...seatForm, count: e.target.value })}
                        required
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setSeatForm({ ...seatForm, prefix: e.target.value })}
                        required
                      />
                    </div>
